...
int MaxGrea(int a, int b)
{
	...
	{
		return MaxGrea(b, a%b);
	}
}
GradeScore reduction(GradeScore &result)
{
	if (result.down < 0)
	{
	...
	{
		int x = MaxGrea(abs(result.up), abs(result.down));
		result.up /= x;
		result.down /= x;
	}
}

GradeScore ADD(GradeScore a, GradeScore b)
{
	GradeScore c;
...
GradeScore Sub(GradeScore a, GradeScore b)
{
	GradeScore c;
